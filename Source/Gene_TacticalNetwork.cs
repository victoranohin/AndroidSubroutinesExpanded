...
{
    public class Gene_TacticalNetwork : Gene
    {
        ...
        private void InitializeHediff()
        {
            if (hediffDef == null)
            {
                hediffDef = DefDatabase<HediffDef>.GetNamedSilentFail("ASE_TacticalNetworkHediff");
            }
        }
        ...
        private int CountAndroidsWithGene()
        {
            ...
            foreach (Pawn p in pawns)
            {
                ...
                {
                    if (gene.def.defName == "ASE_TacticalNetwork")
                    {
                        count++;
                        break;
                    }
        ...
        private void UpdateAllAndroidsWithGene()
        {
            ...
            foreach (Pawn p in pawns)
            {
                ...
                foreach (Gene gene in p.genes.GenesListForReading)
                {
                    Gene_TacticalNetwork tactical = gene as Gene_TacticalNetwork;
                    if (tactical != null)
                    {
                        tactical.UpdateCombatBonus();
                        break;
                    }
                }
            }
